...
import { getBlock, getTransaction, getTransactions } from '../pugdag-api-client.js';
...
const TransactionInfo = () => {
    ...
    const getAmountFromOutputs = (outputs, i) => {
        for (const o of outputs) {
            if (o.index == i) {
                return isNaN(o.amount) ? 0 : o.amount / 100000000;
            }
        }
    }
                                ...
                                {showTxFee && <Row className="blockinfo-row">
                                    ...
                                    <Col className="blockinfo-value-mono" lg={10}>
                                        {txInfo && additionalTxInfo &&
                                            <>{(txInfo.inputs.map((tx_input) => (getOutputFromIndex(additionalTxInfo[tx_input.previous_outpoint_hash]?.outputs || [], tx_input.previous_outpoint_index)?.amount || 0)).reduce((a, b) => a + b) - (txInfo.outputs?.map((v) => v.amount) || [0]).reduce((a, b) => a + b)) / 100000000} PUG</>
                                        }
                                    </Col>
                                </Row>}
                                            ...
                                            {!!additionalTxInfo && additionalTxInfo[tx_input.previous_outpoint_hash] && <Col sm={12} md={12} lg={3}>
                                                ...
                                                <div className="utxo-value">
                                                    <span className="utxo-amount-minus">-{getOutputFromIndex(additionalTxInfo[tx_input.previous_outpoint_hash]
                                                        .outputs, tx_input.previous_outpoint_index).amount / 100000000}&nbsp;PUG</span>
                                                </div>
                                            </Col>}
                                    ...
                                    (txInfo.outputs || []).map((tx_output) =>
                                        <Row className="utxo-border py-3">
                                            ...
                                            <Col sm={6} md={6} lg={3}>
                                                ...
                                                <div className="utxo-value">
                                                    <span className="utxo-amount">+{numberWithCommas(tx_output.amount / 100000000)}&nbsp;PUG</span>
                                                </div>
                                            </Col>
                                            <Col sm={12} md={12} lg={12}>
